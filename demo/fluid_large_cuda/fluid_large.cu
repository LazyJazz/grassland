...
FluidLarge::FluidLarge(const char *title, int width, int height, const PhysicSettings& physic_settings) {
  ...
  color_frame_ = std::make_unique<vulkan::framework::TextureImage>(core_.get(), width, height, VK_FORMAT_B8G8R8A8_UNORM);
  depth_frame_ = std::make_unique<vulkan::framework::TextureImage>(core_.get(), width, height, VK_FORMAT_D32_SFLOAT,  VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
  stencil_frame_ = std::make_unique<vulkan::framework::TextureImage>(core_.get(), width, height, VK_FORMAT_R32_UINT);

  global_uniform_buffer_ = std::make_unique<vulkan::framework::StaticBuffer<GlobalUniformObject>>(core_.get(), 1);
  instance_info_buffer_ = std::make_unique<vulkan::framework::StaticBuffer<InstanceInfo>>(core_.get(), physic_settings.num_particle);
  ...
  render_node_->AddUniformBinding(global_uniform_buffer_.get(), VK_SHADER_STAGE_VERTEX_BIT);
  render_node_->AddBufferBinding(instance_info_buffer_.get(), VK_SHADER_STAGE_VERTEX_BIT);
  ...
  vertex_buffer_ = std::make_unique<vulkan::framework::StaticBuffer<Vertex>>(core_.get(), vertices.size());
  index_buffer_ = std::make_unique<vulkan::framework::StaticBuffer<uint32_t>>(core_.get(), indices.size());
...
void FluidLarge::OnRender() {
  ...
  render_node_->Draw(vertex_buffer_.get(), index_buffer_.get(), index_buffer_->Size(), 0, instance_infos_.size());
...
void FluidLarge::OnUpdate() {
  ...
  instance_info_buffer_->Upload(instance_infos_.data(), sizeof(InstanceInfo) * instance_infos_.size());
}
