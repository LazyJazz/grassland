...
void Application::OnInit() {
  msaa_sample_count_ =
      core_->Device()->PhysicalDevice().GetMaxUsableSampleCount();
  ...
  render_pass_ = std::make_unique<vulkan::RenderPass>(
      ...
      depth_attachment_references, resolve_attachment_references);
  ...
  framebuffer_ = std::make_unique<vulkan::Framebuffer>(
      ...
      std::vector<VkImageView>{msaa_image_->ImageView(),
                               depth_image_->ImageView(),
                               framebuffer_image_->ImageView()});
}
