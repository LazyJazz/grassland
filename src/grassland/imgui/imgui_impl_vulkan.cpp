...
//  2022-10-04: Vulkan: Added experimental ImGui_ImplVulkan_RemoveTexture() for
//  api symetry. (#914, #5738). 2022-01-20: Vulkan: Added support for
//  ImTextureID as VkDescriptorSet. User need to call
...
static void ImGui_ImplVulkan_CreateFontSampler(
    VkDevice device,
    const VkAllocationCallbacks *allocator) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
  if (bd->FontSampler)
...
static void ImGui_ImplVulkan_CreatePipelineLayout(
    VkDevice device,
    const VkAllocationCallbacks *allocator) {
  ImGui_ImplVulkan_Data *bd = ImGui_ImplVulkan_GetBackendData();
